...
4. Все коммиты задачи остаются в этой ветке до merge через `/git-merge-to-develop` или вручную
...
- После `/close-task` при необходимости выполнить merge в `develop` через `/git-merge-to-develop` или вручную (только с явным подтверждением пользователя). **Удаление feature-ветки не выполняется по умолчанию** — можно предложить пользователю; удалять **только после явного подтверждения**
