...
#### Обновление приложения (баннер «Обновить»)

1. После деплоя новой версии дождаться появления баннера (или вернуть вкладку в фокус / сеть online — `swUpdateController` вызывает `registration.update()`).
2. Нажать **«Обновить»**: кнопка сразу становится `disabled`, подпись → **«Обновляется…»**; повторные клики не запускают повторный `updateSW`.
3. Ожидание: страница перезагружается с новой версией SW (activated).
