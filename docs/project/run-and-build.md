...
Локальный сервер для предпросмотра содержимого `dist/`. **Рекомендуется** для проверки PWA (manifest, SW, офлайн) после настройки соответствующих шагов. Пошаговый чеклист: [pwa-checklist.md](./pwa-checklist.md).
